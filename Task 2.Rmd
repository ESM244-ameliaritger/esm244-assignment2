...
fish <- raw_fish %>%
  ...
  mutate(ignore = as.Date(iconv(paste("01", month, "1800", sep=""), 
         ...
         month_number = as.numeric(lubridate::month(ignore)),
         month = month.abb[month_number], #utilize month.abb function, built into base R
         yr_mo = paste(year, month, sep="-"), #combine year and month columns and separate by -
         yr_mo_day = paste(year, month_number, day, sep="-"), #combine all date columns and separate by -
         date = as.Date(yr_mo_day), #tell R this is a date
         year_month = tsibble::yearmonth(yr_mo)) %>% #tell R this is a date
  select("year", "year_month", "date", "value") #remove unnecessary variables
